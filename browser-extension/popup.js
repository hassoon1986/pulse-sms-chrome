...
webview.onload = function() {
    //console.log("requesting account id");
    webview.contentWindow.postMessage("requesting account id", "*");

    chrome.storage.sync.get({
      displayOptionsBar: true
    }, function(items) {
      if (typeof items !== "undefined" && items.displayOptionsBar) {
        document.getElementById('toolbar').style.display = 'block';
        document.getElementById('messenger').style.height = '474px';
      }
    });
    ...
    document.getElementById('popup').addEventListener("click", function() {
      window.close();
      window.open("https://pulsesms.app", "popup", "toolbar,scrollbars,resizable,top=20,left=20,width=415,height=600");
    });
